...
/** Maximum number if outgoing fundamentalnodes */
static const int MAX_OUTBOUND_FUNDAMENTALNODE_CONNECTIONS = 20;
/** -listen default */
...
public:
    ...
    bool OpenNetworkConnection(const CAddress& addrConnect, bool fCountFailure, CSemaphoreGrant *grantOutbound = NULL, const char *strDest = NULL, bool fOneShot = false, bool fFeeler = false, bool fAddnode = false, bool fConnectToMasternode = false, bool fConnectToFundamentalnode = false);
    bool OpenMasternodeConnection(const CAddress& addrConnect);
    bool OpenFundamentalnodeConnection(const CAddress& addrConnect);
    ...
    bool IsFundamentalnodeOrDisconnectRequested(const CService& addr);
    ...
    bool AddPendingFundamentalnode(const CService& addr);
...
private:
    ...
    void ThreadOpenFundamentalnodeConnections();
    ...
    std::vector<CService> vPendingFundamentalnodes;
    CCriticalSection cs_vPendingFundamentalnodes;
    ...
    CSemaphore *semFundamentalnodeOutbound;
    int nMaxConnections;
    int nMaxOutbound;
    int nMaxAddnode;
    int nMaxFeeler;
    ...
    std::thread threadOpenFundamentalnodeConnections;
...
public:
    ...
    bool fMasternode;
    // If 'true' this node will be disconnected on CFundamentalnodeMan::ProcessFundamentalnodeConnections()
    bool fFundamentalnode;
    CSemaphoreGrant grantOutbound;
    CSemaphoreGrant grantMasternodeOutbound;
    CSemaphoreGrant grantFundamentalnodeOutbound;
