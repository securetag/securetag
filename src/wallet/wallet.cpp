...
/** 
 * Fees smaller than this (in tags) are considered zero fee (for transaction creation)
...
CAmount CWallet::GetStake() const
{
    ...
    nTotal = nTotal - ((masternodeConfig.getCount() * 5000) - ((fundamentalnodeConfig.getCount() * 10000)));
    return nTotal;
}
...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                } else if(nCoinType == ONLY_FN) {
                    found = pcoin->tx->vout[i].nValue == FN_MAGIC_AMOUNT;
                ...
                if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO &&
                    (!IsLockedCoin((*it).first, i) || nCoinType == ONLY_1000 || nCoinType == ONLY_FN) &&
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    for (const auto& out : vCoins)
    {
        ...
        if(fFundamentalnodeMode && out.tx->tx->vout[out.i].nValue == FN_MAGIC_AMOUNT) continue; //fundamentalnode input
        if(nValueRet + out.tx->tx->vout[out.i].nValue <= nValueMax){
            CTxIn txin = CTxIn(out.tx->GetHash(),out.i);
...
bool CWallet::GetFundamentalnodeOutpointAndKeys(COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet, const std::string& strTxHash, const std::string& strOutputIndex)
{
    // wait for reindex and/or import to finish
    if (fImporting || fReindex) return false;
    ...
    // Find possible candidates
    std::vector<COutput> vPossibleCoins;
    AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_FN);
    if(vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetFundamentalnodeOutpointAndKeys -- Could not locate any valid fundamentalnode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetFundamentalnodeOutpointAndKeys -- Could not locate specified fundamentalnode vin\n");
    return false;
}
...
bool CWallet::CreateTransaction(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet,
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl* coinControl, bool sign, AvailableCoinsType nCoinType, bool fUseInstantSend, bool IsFundamentalNodePayment)
{
    ...
    if (IsFundamentalNodePayment) {
        LogPrintf ("Committing FN burn tx \n");
    }
            ...
            while (true)
            {
                ...
                if(IsFundamentalNodePayment){
                    for (const auto& recipient : vecSend)
                    {
                        ...
                        if(nValue < FUNDAMENTALNODE_AMOUNT){
                            return false;
                        }

                        CTxOut txout(FN_MAGIC_AMOUNT, recipient.scriptPubKey);
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 SECURETAG."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-securetag-address
                        CScript scriptChange;
                              ...
                              std::vector<CWalletTx*> &vwtxPrev)
{
    ...
    CTxOut txoutFundamentalnode;
    std::vector<CTxOut> voutSuperblock;
    int nHeight = chainActive.Tip()->nHeight + 1;
    FillBlockPayments(txNew, nHeight, blockReward, txoutMasternode, txoutFundamentalnode, voutSuperblock);
    AdjustMasternodePayment(txNew, txoutMasternode, txoutFundamentalnode);
    LogPrintf("CreateCoinStake -- nBlockHeight %d blockReward %lld txoutMasternode %s txoutFundamentalnode %s txNew %s",
              nHeight, blockReward, txoutMasternode.ToString(), txoutFundamentalnode.ToString(), txNew.ToString());
