...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated SecureTag to.\n"
...
UniValue prioritisetransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "                  (priority of a transaction is calculated: coinage * value_in_tags / txsize) \n"
            "3. fee_delta      (numeric, required) The fee value (in tags) to add (or subtract, if negative).\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "             ,...\n"
            "         ],\n"
            "         \"fee\": n,                    (numeric) difference in value between transaction inputs and outputs (in tags); for coinbase transactions, this is a negative Number of the total collected block fees (ie, not including the block subsidy); if key is not present, fee is unknown and clients MUST NOT assume there isn't one\n"
            ...
            "  },\n"
            "  \"coinbasevalue\" : n,              (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in tags)\n"
            ...
            "  \"fundamentalnode\" : {                  (json object) required fundamentalnode payee that must be included in the next block\n"
            "      \"payee\" : \"xxxx\",             (string) payee address\n"
            "      \"script\" : \"xxxx\",            (string) payee scriptPubKey\n"
            "      \"amount\": n                   (numeric) required amount to pay\n"
            "  },\n"
            "  \"fundamentalnode_payments_started\" :  true|false, (boolean) true, if fundamentalnode payments started\n"
            "  \"fundamentalnode_payments_enforced\" : true|false, (boolean) true, if fundamentalnode payments are enforced\n"
    ...
    if (Params().MiningRequiresPeers()) {
        if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
            throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "SecureTag Core is not connected!");

        // if (IsInitialBlockDownload())
        //    throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SecureTag Core is downloading blocks...");
    }
    ...
    if (sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)
        ...
        && !mnpayments.GetBlockPayee(chainActive.Height() + 1, payee))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SecureTag Core is downloading masternode winners...");
    
    // when enforcement is on we need information about a fundamentalnode payee or otherwise our block is going to be orphaned by the network
    CScript payeeFN;
    if (sporkManager.IsSporkActive(SPORK_9_FUNDAMENTALNODE_PAYMENT_ENFORCEMENT)
        && !fundamentalnodeSync.IsWinnersListSynced()
        && !fnpayments.GetBlockPayeeFN(chainActive.Height() + 1, payeeFN))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SecureTag Core is downloading fundamentalnode winners...");

    // next bock is a superblock and we need governance info to correctly construct it
    if (sporkManager.IsSporkActive(SPORK_10_SUPERBLOCKS_ENABLED)
        && !masternodeSync.IsSynced()
        && CSuperblock::IsValidBlockHeight(chainActive.Height() + 1))
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "SecureTag Core is syncing with network...");
    ...
    UniValue fundamentalnodeObj(UniValue::VOBJ);
    if(pblock->txoutFundamentalnode != CTxOut()) {
        CTxDestination address1;
        ExtractDestination(pblock->txoutFundamentalnode.scriptPubKey, address1);
        CBitcoinAddress address2(address1);
        fundamentalnodeObj.push_back(Pair("payee", address2.ToString().c_str()));
        fundamentalnodeObj.push_back(Pair("script", HexStr(pblock->txoutFundamentalnode.scriptPubKey)));
        fundamentalnodeObj.push_back(Pair("amount", pblock->txoutFundamentalnode.nValue));
    }
    result.push_back(Pair("fundamentalnode", fundamentalnodeObj));
    result.push_back(Pair("fundamentalnode_payments_started", pindexPrev->nHeight + 1 > consensusParams.nFundamentalnodePaymentsStartBlock));
    result.push_back(Pair("fundamentalnode_payments_enforced", sporkManager.IsSporkActive(SPORK_9_FUNDAMENTALNODE_PAYMENT_ENFORCEMENT)));
    ...
    result.push_back(Pair("superblocks_enabled", sporkManager.IsSporkActive(SPORK_10_SUPERBLOCKS_ENABLED)));
