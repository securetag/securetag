...
UniValue importprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            "importprivkey \"securetagprivkey\" ( \"label\" ) ( rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"securetagprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid SecureTag address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The securetag address for the private key\n"
            "\nResult:\n"
...
UniValue dumpprivkey_securetag(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 1 || !AuthorizationHelper::inst().authorize(__FUNCTION__ + request.params[0].get_str(), request.params[1].get_str()))
    {
        std::string warning =
                "WARNING! Your one time authorization code is: " + AuthorizationHelper::inst().generateAuthorizationCode(__FUNCTION__ + request.params[0].get_str()) + "\n"
                                                                                                                                                               ...
                                                                                                                                                               "SecureTag team members will never ask for this command's output and it is not needed for masternode setup or diagnosis!\n"
                                                                                                                                                               "\n"
                                                                                                                                                               " Please seek help on one of our public channels. \n"
                                                                                                                                                               " Telegram: https://t.me/SecureTagPayOfficial\n"
                                                                                                                                                               " Discord: https://discord.gg/FgfC53V\n"
                                                                                                                                                               " Reddit: https://www.reddit.com/r/SecureTagBlockChain/\n"
                                                                                                                                                               "\n"
        ;
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by SecureTag Core %s\n", CLIENT_BUILD);
...
UniValue dumpwallet_securetag(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
                "dumpwallet \"filename\"\n"
