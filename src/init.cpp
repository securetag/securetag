...
void PrepareShutdown()
{
    ...
    RenameThread("securetag-shutoff");
    ...
    if (!fLiteMode) {
        ...
        CFlatDB<CFundamentalnodeMan> flatdb2("fncache.dat", "magicFundamentalnodeCache");
        flatdb2.Dump(fnodeman);
        CFlatDB<CMasternodePayments> flatdb3("mnpayments.dat", "magicMasternodePaymentsCache");
        flatdb3.Dump(mnpayments);
        CFlatDB<CFundamentalnodePayments> flatdb4("fnpayments.dat", "magicFundamentalnodePaymentsCache");
        flatdb4.Dump(fnpayments);
        CFlatDB<CGovernanceManager> flatdb5("governance.dat", "magicGovernanceCache");
        flatdb5.Dump(governance);
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "securetag (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, fundamentalnode, fnpayments, fnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all SecureTag specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify fundamentalnode configuration file (default: %s)"), "fundamentalnode.conf"));
    strUsage += HelpMessageOpt("-mnconflock=<n>", strprintf(_("Lock fundamentalnodes from fundamentalnode configuration file (default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N SECURETAG anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/securetagpay/securetag>";
    const std::string URL_WEBSITE = "<https://securetagpay.org>";
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("securetag-loadblk");
...
/** Sanity checks
 *  Ensure that SecureTag Core is running in a usable environment with all
...
void InitParameterInteraction()
{
    ...
    if (GetBoolArg("-fundamentalnode", false)) {
        // fundamentalnodes MUST accept connections from outside
        ForceSetArg("-listen", "1");
        LogPrintf("%s: parameter interaction: -fundamentalnode=1 -> setting -listen=1\n", __func__);
        if (GetArg("-maxconnections", DEFAULT_MAX_PEER_CONNECTIONS) < DEFAULT_MAX_PEER_CONNECTIONS) {
            // fundamentalnodes MUST be able to handle at least DEFAULT_MAX_PEER_CONNECTIONS connections
            ForceSetArg("-maxconnections", itostr(DEFAULT_MAX_PEER_CONNECTIONS));
            LogPrintf("%s: parameter interaction: -fundamentalnode=1 -> setting -maxconnections=%d\n", __func__, DEFAULT_MAX_PEER_CONNECTIONS);
        }
    }
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("SecureTag Core version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single SecureTag Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    fFundamentalnodeMode = GetBoolArg("-fundamentalnode", false);
    // TODO: fundamentalnode should have no wallet

    //lite mode disables all SecureTag-specific functionality
    fLiteMode = GetBoolArg("-litemode", false);

    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, all SecureTag-specific functionality is disabled."));
    }

    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/securetagpay/securetag/pull/1817 and https://github.com/securetagpay/securetag/pull/1743
    ...
    /*if(fLiteMode && fMasternodeMode || fFundamentalnodeMode) {
        return InitError(_("You can not start a masternode/fundamentalnode in lite mode."));
    }*/
    ...
    if(fFundamentalnodeMode) {
        LogPrintf("FUNDAMENTALNODE:\n");

        std::string strFundamentalNodePrivKey = GetArg("-fundamentalnodeprivkey", "");
        if(!strFundamentalNodePrivKey.empty()) {
            if(!CMessageSigner::GetKeysFromSecret(strFundamentalNodePrivKey, activeFundamentalnode.keyFundamentalnode, activeFundamentalnode.pubKeyFundamentalnode))
                return InitError(_("Invalid fundamentalnodeprivkey. Please see documenation."));

            LogPrintf("  pubKeyFundamentalnode: %s\n", CBitcoinAddress(activeFundamentalnode.pubKeyFundamentalnode.GetID()).ToString());
        } else {
    ...
    LogPrintf("Using fundamentalnode config file %s\n", GetFundamentalnodeConfigFile().string());
    ...
    if(GetBoolArg("-fnconflock", true) && pwalletMain && (fundamentalnodeConfig.getCount() > 0)) {
        LOCK(pwalletMain->cs_wallet);
        LogPrintf("Locking Fundamentalnodes:\n");
        uint256 mnTxHash;
        uint32_t outputIndex;
        for (const auto& mne : fundamentalnodeConfig.getEntries()) {
            mnTxHash.SetHex(mne.getTxHash());
            outputIndex = (uint32_t)atoi(mne.getOutputIndex());
            COutPoint outpoint = COutPoint(mnTxHash, outputIndex);
            // don't lock non-spendable outpoint (i.e. it's already spent or it's not from this wallet at all)
    ...
    if (!fLiteMode) {
        ...
        strDBName = "fncache.dat";
        CFlatDB<CFundamentalnodeMan> flatdb2(strDBName, "magicFundamentalnodeCache");
        if(!flatdb2.Load(fnodeman)) {
            return InitError(_("Failed to load fundamentalnode cache from") + "\n" + (pathDB / strDBName).string());
        }
        ...
        if(mnodeman.size()) {
            ...
            if(!flatdb3.Load(mnpayments)) {
                return InitError(_("Failed to load masternode payments cache from") + "\n" + (pathDB / strDBName).string());
            }

            if (fnodeman.size()) {
                strDBName = "fnpayments.dat";
                uiInterface.InitMessage(_("Loading fundamentalnode payment cache..."));
                CFlatDB<CFundamentalnodePayments> flatdb4(strDBName, "magicFundamentalnodePaymentsCache");
                if(!flatdb4.Load(fnpayments)) {
                    return InitError(_("Failed to load fundamentalnode payments cache from") + "\n" + (pathDB / strDBName).string());
                }
            }

            strDBName = "governance.dat";
            uiInterface.InitMessage(_("Loading governance cache..."));
            CFlatDB<CGovernanceManager> flatdb5(strDBName, "magicGovernanceCache");
            if(!flatdb5.Load(governance)) {
                return InitError(_("Failed to load governance cache from") + "\n" + (pathDB / strDBName).string());
            }
            governance.InitOnLoad();
        } else {
            uiInterface.InitMessage(_("Masternode/Fundamentalnode cache is empty, skipping payments and governance cache..."));
        }
    ...
    // ********************************************************* Step 11c: update block tip in SecureTag modules
    ...
    // ********************************************************* Step 11d: start securetag-ps-<smth> threads
