...
UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print SecureTag address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
...
UniValue fundamentalnode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "outputs") {
        ...
        // Find possible candidates
        std::vector<COutput> vPossibleCoins;
        pwalletMain->AvailableCoins(vPossibleCoins, true, NULL, false, ONLY_FN);
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    ...
    { "securetag",               "sentinelping",           &sentinelping,           true,  {} },
