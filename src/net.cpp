...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (SecureTag users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "SecureTag Core " + FormatFullVersion();
...
void CConnman::ProcessOneShot()
{
    ...
    if (grant) {
        if (!OpenNetworkConnection(addr, false, &grant, strDest.c_str(), true, true))
            AddOneShot(strDest);
    }
}
...
void CConnman::ThreadOpenFundamentalnodeConnections()
{
    // Connecting to specific addresses, no masternode connections available
    if (IsArgSet("-connect") && mapMultiArgs.at("-connect").size() > 0)
        return;
    ...
    while (!interruptNet)
    {
        ...
        CSemaphoreGrant grant(*semFundamentalnodeOutbound);
        if (interruptNet)
            return;

        // NOTE: Process only one pending masternode at a time

        LOCK(cs_vPendingFundamentalnodes);
        if (vPendingFundamentalnodes.empty()) {
            // nothing to do, keep waiting
            continue;
        }

        const CService addr = vPendingFundamentalnodes.front();
        vPendingFundamentalnodes.erase(vPendingFundamentalnodes.begin());
        if (IsFundamentalnodeOrDisconnectRequested(addr)) {
            // nothing to do, try the next one
            continue;
        }

        OpenFundamentalnodeConnection(CAddress(addr, NODE_NETWORK));
        // should be in the list now if connection was opened
        ForNode(addr, CConnman::AllNodes, [&](CNode* pnode) {
            if (pnode->fDisconnect) {
                return false;
            }
            grant.MoveTo(pnode->grantFundamentalnodeOutbound);
            return true;
        });
    }
}
...
bool CConnman::OpenNetworkConnection(const CAddress& addrConnect, bool fCountFailure, CSemaphoreGrant *grantOutbound, const char *pszDest, bool fOneShot, bool fFeeler, bool fAddnode, bool fConnectToMasternode, bool fConnectToFundamentalnode)
{
    //
    ...
    if (fConnectToFundamentalnode)
        pnode->fFundamentalnode = true;
...
bool CConnman::OpenFundamentalnodeConnection(const CAddress &addrConnect) {
    return OpenNetworkConnection(addrConnect, false, NULL, NULL, false, false, false, false, true);
}
        ...
        addrman(Params().AllowMultiplePorts())
{
    ...
    semFundamentalnodeOutbound = NULL;
    nMaxConnections = 0;
    nMaxOutbound = 0;
    nMaxAddnode = 0;
    nBestHeight = 0;
...
bool CConnman::Start(CScheduler& scheduler, std::string& strNodeError, Options connOptions)
{
    ...
    if (semFundamentalnodeOutbound == NULL) {
        // initialize semaphore
        semFundamentalnodeOutbound = new CSemaphore(MAX_OUTBOUND_FUNDAMENTALNODE_CONNECTIONS);
    }
    ...
    // Initiate fundamentalnode connections
    threadOpenFundamentalnodeConnections = std::thread(&TraceThread<std::function<void()> >, "fncon", std::function<void()>(std::bind(&CConnman::ThreadOpenFundamentalnodeConnections, this)));
...
void CConnman::Interrupt()
{
    ...
    if (semFundamentalnodeOutbound) {
        for (int i=0; i<MAX_OUTBOUND_FUNDAMENTALNODE_CONNECTIONS; i++) {
            semFundamentalnodeOutbound->post();
        }
    }
}
...
void CConnman::Stop()
{
    ...
    if (threadOpenFundamentalnodeConnections.joinable())
        threadOpenFundamentalnodeConnections.join();
    ...
    semMasternodeOutbound = NULL;
    delete semFundamentalnodeOutbound;
    semFundamentalnodeOutbound = NULL;
}
...
bool CConnman::AddPendingFundamentalnode(const CService& service)
{
    LOCK(cs_vPendingFundamentalnodes);
    for(std::vector<CService>::const_iterator it = vPendingFundamentalnodes.begin(); it != vPendingFundamentalnodes.end(); ++it) {
        if (service == *it)
            return false;
    }

    vPendingFundamentalnodes.push_back(service);
    return true;
}
...
bool CConnman::IsFundamentalnodeOrDisconnectRequested(const CService& addr) {
    return ForNode(addr, AllNodes, [](CNode* pnode){
        return pnode->fFundamentalnode || pnode->fDisconnect;
    });
}
