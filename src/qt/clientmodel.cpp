    ...
    pollTimer(0)
{
    ...
    pollFnTimer = new QTimer(this);
    connect(pollFnTimer, SIGNAL(timeout()), this, SLOT(updateFnTimer()));
    // no need to update as frequent as data for balances/txes/blocks
    pollFnTimer->start(MODEL_UPDATE_DELAY * 4);
...
QString ClientModel::getFundamentalnodeCountString() const
{
    ...
    return tr("Total: %1 (Enabled: %2)")
            .arg(QString::number((int)fnodeman.size()))
            .arg(QString::number((int)fnodeman.CountEnabled()));
            // .arg(QString::number((int)mnodeman.CountByIP(NET_IPV4)))
            // .arg(QString::number((int)mnodeman.CountByIP(NET_IPV6)))
            // .arg(QString::number((int)mnodeman.CountByIP(NET_TOR)));
}
...
void ClientModel::updateFnTimer()
{
    ...
    if (cachedFundamentalnodeCountString != newFundamentalnodeCountString)
    {
        ...
        Q_EMIT strFundamentalnodesChanged(cachedFundamentalnodeCountString);
    }
}
