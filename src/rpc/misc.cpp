...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total securetag balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "securetag|privatesend|instantsend|masternode|fundamentalnode|spork|keepass|mnpayments|fnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use '+' to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "securetag")
            + HelpExampleRpc("debug", "securetag+net")
        );
...
UniValue fnsync(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            "fnsync [status|next|reset]\n"
            "Returns the sync status, updates to the next step or resets it entirely.\n"
        );
    ...
    if(strMode == "status") {
        UniValue objStatus(UniValue::VOBJ);
        objStatus.push_back(Pair("AssetID", fundamentalnodeSync.GetAssetID()));
        objStatus.push_back(Pair("AssetName", fundamentalnodeSync.GetAssetName()));
        objStatus.push_back(Pair("AssetStartTime", fundamentalnodeSync.GetAssetStartTime()));
        objStatus.push_back(Pair("Attempt", fundamentalnodeSync.GetAttempt()));
        objStatus.push_back(Pair("IsBlockchainSynced", fundamentalnodeSync.IsBlockchainSynced()));
        objStatus.push_back(Pair("IsFundamentalnodeListSynced", fundamentalnodeSync.IsFundamentalnodeListSynced()));
        objStatus.push_back(Pair("IsWinnersListSynced", fundamentalnodeSync.IsWinnersListSynced()));
        objStatus.push_back(Pair("IsSynced", fundamentalnodeSync.IsSynced()));
        objStatus.push_back(Pair("IsFailed", fundamentalnodeSync.IsFailed()));
        return objStatus;
    }

    if(strMode == "next")
    {
        fundamentalnodeSync.SwitchToNextAsset(*g_connman);
        return "sync updated to " + fundamentalnodeSync.GetAssetName();
    }
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given securetag address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The securetag address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The securetag address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: SecureTag address and we have full public key:
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are securetag addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) securetag address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The securetag address to use for the signature.\n"
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "    \"satoshis\"  (number) The difference of tags\n"
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "    \"satoshis\"  (number) The number of tags of the output\n"
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"balance\"  (string) The current balance in tags\n"
            "  \"received\"  (string) The total number of tags received (including change)\n"
            "}\n"
            "\nExamples:\n"
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* SecureTag features */
    { "securetag",               "mnsync",                 &mnsync,                 true,  {} },
    { "securetag",               "fnsync",                 &fnsync,                 true,  {} },
    { "securetag",               "spork",                  &spork,                  true,  {"value"} },
