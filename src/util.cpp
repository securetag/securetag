...
//SecureTag only features
bool fFundamentalnodeMode = false;
...
const char * const BITCOIN_CONF_FILENAME = "securetag.conf";
const char * const BITCOIN_PID_FILENAME = "securetagd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "securetag" is a composite category enabling all SecureTag-related debug output
                if(ptrCategory->count(std::string("securetag"))) {
...
#else
    const char* pszModule = "securetag";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SecureTag
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SecureTag
    // Mac: ~/Library/Application Support/SecureTag
    // Unix: ~/.securetag
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SecureTag";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SecureTag";
#else
    // Unix
    return pathRet / ".securetag";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty securetag.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override securetag.conf
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitcoin Core") == std::string::npos) {
        ...
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2017, 2019) + "The SecureTag Core developers \n";
    }
