...
#if defined(NDEBUG)
# error "SecureTag Core cannot be compiled without assertions."
#endif
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        SecureTag Related Inventory Messages
    ...
    */
    ...
    case MSG_FUNDAMENTALNODE_PAYMENT_VOTE:
        return fnpayments.mapFundamentalnodePaymentVotes.count(inv.hash);
    ...
    case MSG_FUNDAMENTALNODE_PAYMENT_BLOCK:
        {
            BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
            return mi != mapBlockIndex.end() && fnpayments.mapFundamentalnodeBlocks.find(mi->second->nHeight) != fnpayments.mapFundamentalnodeBlocks.end();
        }
    ...
    case MSG_FUNDAMENTALNODE_ANNOUNCE:
        return fnodeman.mapSeenFundamentalnodeBroadcast.count(inv.hash) && !fnodeman.IsFnbRecoveryRequested(inv.hash);
    ...
    case MSG_FUNDAMENTALNODE_VERIFY:{
        return fnodeman.mapSeenFundamentalnodeVerification.count(inv.hash);
    }
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        LogPrint("net", "ProcessGetData -- inv = %s\n", inv.ToString());
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!push && inv.type == MSG_FUNDAMENTALNODE_PAYMENT_VOTE) {
                    if(fnpayments.HasVerifiedPaymentVote(inv.hash)) {
                        connman.PushMessage(pfrom, msgMaker.Make(NetMsgType::FUNDAMENTALNODEPAYMENTVOTE, fnpayments.mapFundamentalnodePaymentVotes[inv.hash]));
                        push = true;
                    }
                }
                ...
                if (!push && inv.type == MSG_FUNDAMENTALNODE_PAYMENT_BLOCK) {
                    BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
                    LOCK(cs_mapFundamentalnodeBlocks);
                    if (mi != mapBlockIndex.end() && fnpayments.mapFundamentalnodeBlocks.count(mi->second->nHeight)) {
                        BOOST_FOREACH(CFundamentalnodePayee& payee, fnpayments.mapFundamentalnodeBlocks[mi->second->nHeight].vecPayeesFN) {
                            std::vector<uint256> vecVoteHashes = payee.GetVoteHashes();
                            BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                                if(fnpayments.HasVerifiedPaymentVote(hash)) {
                                    connman.PushMessage(pfrom, msgMaker.Make(NetMsgType::FUNDAMENTALNODEPAYMENTVOTE, fnpayments.mapFundamentalnodePaymentVotes[hash]));
                                }
                            }
                        }
                        push = true;
                ...
                if (!push && inv.type == MSG_FUNDAMENTALNODE_ANNOUNCE) {
                    if(fnodeman.mapSeenFundamentalnodeBroadcast.count(inv.hash)){
                        connman.PushMessage(pfrom, msgMaker.Make(NetMsgType::FNANNOUNCE, fnodeman.mapSeenFundamentalnodeBroadcast[inv.hash].second));
                        push = true;
                    }
                }
                ...
                if (!push && inv.type == MSG_FUNDAMENTALNODE_PING) {
                    if(fnodeman.mapSeenFundamentalnodePing.count(inv.hash)) {
                        connman.PushMessage(pfrom, msgMaker.Make(NetMsgType::FNPING, fnodeman.mapSeenFundamentalnodePing[inv.hash]));
                        push = true;
                    }
                }
                ...
                if (!push && inv.type == MSG_FUNDAMENTALNODE_VERIFY) {
                    if(fnodeman.mapSeenFundamentalnodeVerification.count(inv.hash)) {
                        connman.PushMessage(pfrom, msgMaker.Make(NetMsgType::FNVERIFY, fnodeman.mapSeenFundamentalnodeVerification[inv.hash]));
                        push = true;
                    }
                }
