                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
...

    {
        ...
        if (nFees > (FUNDAMENTALNODE_AMOUNT - FN_MAGIC_AMOUNT - 0.1*COIN))  {
            ...
            LogPrintf("Fundamentalnode verified, with nFees of %d \n", nFees);
        }
...
if (nPrevHeight == 0){
        return 800000 * COIN;
    } else if(nPrevHeight >= 1 && nPrevHeight <= 6) {
            nSubsidy = 50000 * COIN;
    } else if(nPrevHeight >= 7 && nPrevHeight <= 1000) {
            nSubsidy = .1 * COIN;
    } else if(nPrevHeight >=1001 && nPrevHeight <= 3000) {
            nSubsidy = 1 * COIN;
    } else if(nPrevHeight >= 3001 && nPrevHeight <= 5000) {
            nSubsidy = 2 * COIN;
    } else if(nPrevHeight >= 5001 && nPrevHeight <= 10000) {
            nSubsidy = 10 * COIN;
    } else if(nPrevHeight >= 10001 && nPrevHeight <= 20000) {
            nSubsidy = 20 * COIN;
    } else if(nPrevHeight >= 20001 && nPrevHeight <= 86500) {
            nSubsidy = 16 * COIN;
    } else if(nPrevHeight >= 86501 && nPrevHeight <= 172900) {
            nSubsidy = 14 * COIN;
    } else if(nPrevHeight >= 172901 && nPrevHeight <= 207258) {
            nSubsidy = 12 * COIN;
    } else if(nPrevHeight >= 207259 && nPrevHeight <= 228859) {
            nSubsidy = 10 * COIN;
    } else if(nPrevHeight >= 228860 && nPrevHeight <= 293660) {
            nSubsidy = 14 * COIN;
    } else if(nPrevHeight >= 293661 && nPrevHeight <= 380061) {
            nSubsidy = 12 * COIN;
    } else if(nPrevHeight >= 380062 && nPrevHeight <= 466462) {
            nSubsidy = 10 * COIN;
    } else if(nPrevHeight >= 466463 && nPrevHeight <= 682463) {
            nSubsidy = 8 * COIN;
    } else if(nPrevHeight >= 682464 && nPrevHeight <= 977664) {
            nSubsidy = 4 * COIN;
    } else if(nPrevHeight >= 977665 && nPrevHeight <= 1290785) {
            nSubsidy = 2 * COIN;
}  else {
        nSubsidy = 0 * COIN; }
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
  if (nHeight < 100) {
      return blockValue = 0; 
} else if (nHeight >= 100 && nHeight <= 5000) {
      return blockValue*0.25;
} else if (nHeight >= 5001 && nHeight <= 86500) {
      return blockValue*0.50;
} else if (nHeight >= 86501 && nHeight <= 228859) {
      return blockValue*0.60;
} else if (nHeight >= 228860 && nHeight <= 466462) {
      return blockValue*0.30;
} else if (nHeight >= 466463 && nHeight <= 1290785) {
...
CAmount GetFundamentalnodePayment(int nHeight, CAmount blockValue)
{
...
void ThreadScriptCheck() {
    RenameThread("securetag-scriptch");
                         ...
                         CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    /// SECURETAG: Check superblock start
    ...
    // SECURETAG : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    CAmount expectedReward = nFees + GetBlockSubsidy(pindex->pprev->nHeight,chainparams.GetConsensus());
    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, expectedReward, pindex->nMint, strError)) {
        return state.DoS(0, error("ConnectBlock(SECURETAG): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(coinbaseTransaction, pindex->nHeight, expectedReward, pindex->nMint)) {
        ...
        return state.DoS(0, error("ConnectBlock(SECURETAG): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // SECURETAG : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(SECURETAG): spork is off, skipping transaction locking checks\n");
    }
