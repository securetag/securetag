...
void ThreadCheckPrivateSend(CConnman& connman)
{
    ...
    while (true)
    {
        ...
        fundamentalnodeSync.ProcessTick(connman);

        if(masternodeSync.IsBlockchainSynced() && fundamentalnodeSync.IsBlockchainSynced() && !ShutdownRequested()) {
            ...
            fnodeman.ProcessPendingFnbRequests(connman);
            fnodeman.ProcessPendingFnvRequests(connman);
            ...
            if(nTick % 60 == 0) {
                ...
                fnodeman.ProcessFundamentalnodeConnections(connman);
                fnodeman.CheckAndRemove(connman);
                fnodeman.WarnFundamentalnodeDaemonUpdates();
                fnpayments.CheckAndRemove();
                instantsend.CheckAndRemove();
            }
            if(fMasternodeMode && (nTick % (60 * 5) == 0)) {
                mnodeman.DoFullVerificationStep(connman);
                fnodeman.DoFullVerificationStep(connman);
            }
