...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SecureTag address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no securetag: URI
    if(!uri.isValid() || uri.scheme() != QString("securetag"))
        return false;
...
{
    // Convert securetag:// to securetag:
    //
    //    Cannot handle this later, because securetag:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("securetag://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "securetag:");
    }
...
{
    QString ret = QString("securetag:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open securetag.conf with the associated application */
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "SecureTag Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "SecureTag Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("SecureTag Core (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "SecureTag Core*.lnk"
...
boost::filesystem::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "securetag.desktop";
    return GetAutostartDir() / strprintf("securetag-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a securetag.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=SecureTag Core\n";
        else
            optionFile << strprintf("Name=SecureTag Core (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the SecureTag Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add SecureTag Core app to startup item list
